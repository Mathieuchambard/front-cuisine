...
@Component({
  selector: 'app-coursJanne',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './coursJanne.component.html',
  styleUrl: './coursJanne.component.scss'
})
export class CoursJanneComponent implements OnInit {
  videos: VideoCoursJanne[] = [];
  video!: VideoCoursJanne ;

  constructor(private http: HttpClient) {}

  ngOnInit(): void {
    this.http.get<VideoCoursJanne[]>('/coursJanne/info.json').subscribe(data => {
      this.videos = data;
      this.video = data[0];
      console.log(data)
    });
